...
describe("Flow Queue controls", () => {
  it("offers single Prompt execution and removal for each job", () => {
    ...
    render(<FlowQueueView jobs={[job]} running={false} error="" queueDelaySec={3} onQueueDelayChange={vi.fn()} onExecute={vi.fn()} onStop={vi.fn()} onPause={vi.fn()} onResume={vi.fn()} onRetry={vi.fn()} onCancel={vi.fn()} onExecuteJob={onExecuteJob} onRemove={onRemove} />);
  ...
  it("allows changing the delay between queued jobs", () => {
    ...
    render(<FlowQueueView jobs={[]} running={false} error="" queueDelaySec={3} onQueueDelayChange={onQueueDelayChange} onExecute={vi.fn()} onStop={vi.fn()} onPause={vi.fn()} onResume={vi.fn()} onRetry={vi.fn()} onCancel={vi.fn()} onExecuteJob={vi.fn()} onRemove={vi.fn()} />);

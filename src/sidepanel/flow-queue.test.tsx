...
describe("Flow Queue controls", () => {
  it("offers single Prompt execution and removal for each job", () => {
    ...
    render(<FlowQueueView jobs={[job]} running={false} onExecute={vi.fn()} onStop={vi.fn()} onPause={vi.fn()} onResume={vi.fn()} onRetry={vi.fn()} onCancel={vi.fn()} onExecuteJob={onExecuteJob} onRemove={onRemove} />);

...
export function recordDebugLog(level: DebugLogEntry["level"], stage: string, message: string, details?: Record<string, unknown>) {
  const entry: DebugLogEntry = { id: crypto.randomUUID(), time: new Date().toISOString(), level, stage, message, details };
  ...
  if (level === "error") console.error(`[Flow Companion][${stage}] ${message}`, details ?? ""); else console.info(`[Flow Companion][${stage}] ${message}`, details ?? "");
  return entry;
}

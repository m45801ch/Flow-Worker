...
export function GateReportPanel({ report }: { report?: GateReport }) {
  ...
  return <section className={`output-card gate-report ${report.passed ? "gate-passed" : "gate-blocked"}`}><div className="job-card-header"><h3>{report.passed ? "品質門通過" : "品質門阻擋"}</h3><span className="stage-state">{report.passed ? "通過" : "已阻擋"}</span></div>{report.blockers.length > 0 && <div><b>阻擋項目</b>{report.blockers.map((issue) => <p key={`${issue.code}-${issue.path}`}>{issue.code}: {issue.message}</p>)}</div>}{report.warnings.length > 0 && <div><b>警告</b>{report.warnings.map((issue) => <p key={`${issue.code}-${issue.path}`}>{issue.code}: {issue.message}</p>)}</div>}<pre>{JSON.stringify(report.metrics, null, 2)}</pre></section>;
}

...
describe("storyboard asset references", () => {
  it("matches named assets mentioned in a Cut and carries their reference IDs", () => {
    const jobs = compileStoryboardJobs(storyboard, {
      ...context,
      assetCatalog: [
        { id: "C01", name: "Mara", kind: "character", referenceAssetIds: ["REF-MARA"] },
        { id: "P01", name: "lighthouse", kind: "prop", referenceAssetIds: ["REF-LIGHTHOUSE"] },
      ],
    });
    expect(jobs[0]).toMatchObject({ assetNames: ["Mara", "lighthouse"], assetBindings: expect.arrayContaining(["C01", "S01", "P01"]), inputAssetIds: expect.arrayContaining(["A01", "REF-MARA", "REF-LIGHTHOUSE"]) });
  });
});

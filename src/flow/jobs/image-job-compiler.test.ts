...
describe("image Flow job compiler", () => {
  it("compiles a 16:9 character sheet job with the selected image model", () => {
    ...
    expect(job).toMatchObject({ kind: "character-sheet", outputMode: "image", modelName: "Nano Banana 2", aspectRatio: "16:9", sourceEntityId: "C01", assetBindings: ["C01"] });
  ...
  it("rejects an image job without an explicitly selected model", () => {
    expect(() => compileCharacterSheetJob(character, { ...context, imageModel: "" })).toThrow(/image model/i);
  });
});

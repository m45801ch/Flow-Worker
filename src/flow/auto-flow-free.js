...
(() => {
  ...
  function detectFlowMode() {
    ...
    // 若設定面板同時顯示「圖片／影片」兩個 tab，且沒有 active 狀態，不能用 DOM 順序猜測。
    // 只有看到單一模式按鈕，或看到同一個合併設定按鈕時，才接受文字判斷。
    const modeCandidates = toolbarBtns.filter(el => {
      const t = norm(el.textContent);
      return /^(圖片|图片|image|影片|视频|視頻|video)(\s|$)/i.test(t);
    });
    const combinedCandidates = toolbarBtns.filter(el => {
      const t = norm(el.textContent);
      const hasVideo = videoTextRe.test(t);
      const hasImage = imageTextRe.test(t);
      return hasVideo !== hasImage && t.length < 80;
    });
    if (combinedCandidates.length === 1) {
      const t = norm(combinedCandidates[0].textContent);
      return videoTextRe.test(t) ? "video" : "image";
    }
    if (modeCandidates.length === 1) {
      const t = norm(modeCandidates[0].textContent);
      return videoTextRe.test(t) ? "video" : "image";
    }
    // Strategy 3: Check prompt area only when no ambiguous mode tabs are visible.
    const promptBtns = Array.from(document.querySelectorAll("button")).filter(el => {
      const r = el.getBoundingClientRect();
      if (!(r.width > 0 && r.height > 0)) return false;
      return r.top > bottomThreshold;
    });
    const promptModes = promptBtns.map(el => norm(el.textContent)).filter(t => /veo|banana|🍌/i.test(t));
    if (promptModes.length === 1) return /veo/i.test(promptModes[0]) ? "video" : "image";
    return null;
  }
  ...
  async function ensureOutputMode(kind) {
    // 每項任務都重新讀取底部工具列；Flow 可能在上一個任務完成後重設模式。
    const detected = detectFlowMode();
    flowCurrentMode = detected;
    if (detected === kind) {
    ...
    if (modeTab) {
      ...
      if (!selected) {
        click(modeTab);
        await sleep(1800);
        log("Switched Flow role=tab to", kind, "via", modeTab.id || modeTab.textContent);
      } else {
        log("Flow role=tab already selected:", kind);
      }
      flowCurrentMode = kind;
      return true;
    }
    ...
    if (target) {
      const active = target.getAttribute("aria-pressed") === "true" ||
        ...
        target.getAttribute("data-state") === "on" ||
  ...
  function setAspect() {
    if (!config.aspect) return true;
    const suffix = config.aspect === "16:9" ? "-trigger-LANDSCAPE" : config.aspect === "9:16" ? "-trigger-PORTRAIT" : "";
    if (suffix) {
  ...
  function setModel() {
    ...
    if (selectByText(label)) { log("Model set to", label); return true; }
      ...
      return new Promise(resolve => {
        ...
        const tryFind = () => {
          ...
          const options = queryAllVisible(document).filter(el => {
            ...
            const t = norm(el.textContent);
            // 依照目前要求的模型精確比對，不再寫死 Veo Lite。
            const clean = t.replace(/[\s\-_🎤🔊🎶🎵]/g, " ");
            const wanted = targetLower.replace(/[\s\-_🎤🔊🎶🎵]/g, " ");
            return clean.includes(wanted);
          });
          if (options.length > 0) {
            // 取最小的元素（最精確的匹配）
  ...
  function setImageModel() {
    if (!config.imageModel) return true;
    const map = {
    ...
    if (selectByText(label) || selectFuzzy(label)) { log("Image model set to", label); return true; }
    ...
    if (trigger) {
      ...
      return new Promise(resolve => {
        ...
        const tryFind = () => {
          attempts++;
          const menuItems = Array.from(document.querySelectorAll('[role="menuitem"]')).filter(el => {
            const r = el.getBoundingClientRect();
            return r.width > 20 && r.height > 10 && r.height < 80;
          });
          const options = menuItems.filter(el => norm(el.textContent) === target || norm(el.textContent).includes(target));
          if (options.length > 0) {
            const best = options.sort((a, b) => {
              const ra = a.getBoundingClientRect();
              const rb = b.getBoundingClientRect();
            ...
            click(best);
            log("Image model set to", label, "from role=menuitem (attempt", attempts + ")");
            resolve(true);
          } else if (attempts < 5) {
            setTimeout(tryFind, 600);
          } else {
            log("Image model not found in dropdown:", label, "- available:", JSON.stringify(
              menuItems.map(el => norm(el.textContent).slice(0, 30))
            ));
            try { document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true })); } catch (e) {}
            resolve(false);
          }
  ...
  function setImageMode(mode) {
    ...
    const label = map[m] || m;
    if (selectByText(label) || selectFuzzy(label)) { log("Image mode set to", label); return true; }
    log("Image mode not found in UI:", label);
    return false;
  }
  ...
  function setOutputs(n) {
    ...
    if (selectByText("x" + n)) { log("Outputs set to", n, "(x" + n + ")"); return true; }
    if (selectByText(String(n))) { log("Outputs set to", n); return true; }
    ...
    if (hit) { click(hit); log("Outputs set to", n); return true; }
    log("Outputs not found:", n, "- available:", JSON.stringify(btns.map(b => norm(b.textContent)).filter(t => /^x?[1-4]$/.test(t))));
    return false;
  }
  ...
  async function tryAddMatchedAssets(text, requestedNames = []) {
    const requested = Array.isArray(requestedNames) ? requestedNames.map(n => String(n || "").trim()).filter(Boolean) : [];
    const chars = [...new Set([...requested, ...charsInText(text)])].slice(0, 3);
    if (chars.length === 0) {
    ...
    const norm = s => (s || "").replace(/_/g, " ").replace(/\s+/g, " ").trim().toLowerCase();
    // Google Flow 實際的提示詞新增按鈕是 button[aria-haspopup="dialog"]，內含 i.google-symbols 的 add_2；不能用全頁模糊的 add／添加，否則會誤抓左側「添加媒体」。
    const visible = (el) => { const r = el.getBoundingClientRect(); return r.width > 0 && r.height > 0; };
    const iconText = (el) => Array.from(el.querySelectorAll("i, [class*='symbol'], [class*='icon']")).map(node => (node.textContent || "").trim()).join(" ");
    const isPromptAddButton = (el) => {
      if (!visible(el)) return false;
      const label = ((el.getAttribute("aria-label") || "") + " " + (el.getAttribute("title") || "")).replace(/\s+/g, " ").trim();
      const text = (el.textContent || "").replace(/\s+/g, " ").trim();
      const hasAddIcon = /(^|\s)add_2(\s|$)/i.test(iconText(el)) || /(^|\s)add_2(\s|$)/i.test(text);
      return el.tagName === "BUTTON" && (el.getAttribute("aria-haspopup") === "dialog" && hasAddIcon || /^(\+|add_2|新增素材|加入素材)$/i.test(label));
    };
    let plus = null;
    let node = ta;
    for (let depth = 0; node && depth < 8 && !plus; depth++, node = node.parentElement) {
      plus = Array.from(node.querySelectorAll("button")).find(isPromptAddButton) || null;
    }
    if (!plus) {
      const taR = ta.getBoundingClientRect();
      plus = Array.from(document.querySelectorAll("button")).filter(isPromptAddButton).sort((a, b) => {
        const ar = a.getBoundingClientRect();
        const br = b.getBoundingClientRect();
        return Math.hypot(ar.left - taR.left, ar.top - taR.top) - Math.hypot(br.left - taR.left, br.top - taR.top);
      })[0] || null;
    }
    if (!plus) { log("Add-asset (+) button not found near prompt; expected button[aria-haspopup=dialog] with add_2"); return; }
    log("Found Flow prompt asset button:", plus.getAttribute("aria-haspopup"), plus.getAttribute("aria-expanded"), iconText(plus));
    // Google Flow 實際素材庫根節點是 div[role="dialog"][data-state="open"]，內含 #add-menu-input 與 [role="option"]。
    function findPicker() {
      const dialogs = Array.from(document.querySelectorAll("dialog, [role='dialog'], [aria-modal='true']")).filter(visible);
      const matched = dialogs.find(el => el.querySelector("#add-menu-input") || Array.from(el.querySelectorAll("button")).some(b => (b.textContent || "").replace(/\s+/g, " ").trim() === "添加到提示"));
      if (matched) return matched;
      return dialogs[0] || null;
    }
    // 僅在素材庫 dialog 內精確找「添加到提示」，避免選到提示詞區或左側「添加媒体」。
    function findConfirmBtn(picker) {
      if (!picker) return null;
      const btns = Array.from(picker.querySelectorAll("button")).filter(visible);
      return btns.find(b => (b.textContent || "").replace(/\s+/g, " ").trim() === "添加到提示") || btns.find(b => /^(Done|Confirm|OK|Select)$/i.test((b.textContent || "").replace(/\s+/g, " ").trim())) || null;
    }
    function findPickerSearch(picker) {
      if (!picker) return null;
      const exact = picker.querySelector("#add-menu-input");
      if (exact && visible(exact)) return exact;
      const inputs = Array.from(picker.querySelectorAll("input, [contenteditable='true']"));
      return inputs.find(el => /search|搜尋|搜索|資源|resource|asset/i.test((el.getAttribute("placeholder") || "") + " " + (el.getAttribute("aria-label") || "") + " " + (el.getAttribute("title") || ""))) || inputs[0] || null;
    }
    async function searchPicker(picker, name) {
      const input = findPickerSearch(picker);
      if (!input) return;
      setNativeValue(input, name);
      await sleep(1200);
    }
    const cleanAssetLabel = (value) => norm(value).replace(/\s+(圖片|图片|圖像|image|人物|角色|視頻|视频|video)$/i, "").trim();
    // 優先點擊 role=option，其次才使用可互動素材卡片 role=button；不再掃描整個 dialog 的任意 div。
    function findCharInPicker(picker, name) {
      ...
      const matches = (el) => {
        const label = cleanAssetLabel(labelOf(el));
        return label === nn || label.includes(nn);
      };
      const options = Array.from(picker.querySelectorAll("[role='option']")).filter(visible);
      const option = options.find(matches);
      if (option) return option;
      const cards = Array.from(picker.querySelectorAll("[role='button'][tabindex='0'], [data-tile-id]")).filter(visible);
    ...
    for (const name of chars) {
      ...
      // 先用卡片文字／替代文字找素材；若素材庫尚未顯示，使用搜尋框再找一次。
      let hit = findCharInPicker(picker, name);
      if (!hit) {
        await searchPicker(picker, name);
  ...
  function variantLabelFor(item, index) {
    const labels = Array.isArray(item.outputVariantLabels) ? item.outputVariantLabels.map(label => String(label || "").trim()).filter(Boolean) : [];
    if (labels.length > 0) return labels[index] || `${labels[labels.length - 1]}-${index + 1}`;
    return Number(item.outputCount || 1) > 1 ? String(index + 1) : "";
  }
  function outputFileName(item, index, extension) {
    const rawBase = String(item.outputName || item.sourceEntityName || `image-${item.id + 1}`).replace(/[\\/:*?"<>|]/g, "_").replace(/\s+/g, " ").trim() || `image-${item.id + 1}`;
    const variant = variantLabelFor(item, index);
    const suffix = variant && !rawBase.toLocaleLowerCase().endsWith(`-${variant}`.toLocaleLowerCase()) ? `-${variant}` : "";
    return `${rawBase}${suffix}.${extension}`;
  }

  async function autoDownload(url, item) {
    const isImage = config.mode === "text2image" || config.mode === "image2image" || /\.(png|jpg|jpeg|webp)$/i.test(url.split("?")[0]) || /image/i.test(item.text || "");
    const targetRes = isImage ? (config.imageRes || "2k").toLowerCase() : (config.videoRes || "1080p").toLowerCase();
    const outputName = String(item.outputName || item.sourceEntityName || "").trim();
    const safeOutputName = outputName.replace(/[\\/:*?"<>|]/g, "_").replace(/\s+/g, " ").trim();
    const outputIndex = (downloadCounts[item.id] || 0) + 1;
    downloadCounts[item.id] = outputIndex;
    ...
    const extensionMatch = filename.match(/\.(png|jpe?g|webp|gif|mp4|webm)$/i);
    const inferredExtension = extensionMatch?.[1] || (isImage ? "png" : "mp4");
    if (isImage && safeOutputName) {
      filename = `${safeFolder}/${outputFileName(item, outputIndex - 1, inferredExtension)}`;
    } else if (config.rename) {
      const ext = inferredExtension;
  ...
  async function processOne(item) {
    ...
    if (panelOpened) {
      ...
      await sleep(1000);
      dumpPanelElements();
      // 面板開啟後每個任務都要重新確認輸出模式；不能只依賴第一次 detectFlowMode，
      // 因為 Flow 初始畫面可能沒有 aria-selected／active 標記而讓偵測結果為 null。
      const targetMode = isImageMode ? "image" : "video";
      const modeReady = await ensureOutputMode(targetMode);
      if (!modeReady) {
        throw new Error(`無法將 Google Flow 切換到${isImageMode ? "圖片" : "影片"}模式，為避免誤生成，已停止此任務`);
      }
      await sleep(1200);
      dumpPanelElements();
      ...
      const modeReady = await ensureOutputMode(isImageMode ? "image" : "video");
      if (!modeReady) throw new Error(`無法將 Google Flow 切換到${isImageMode ? "圖片" : "影片"}模式，為避免誤生成，已停止此任務`);
    }
    // 先設定面板選項（模式、比例、模型、數量、時長）；任何必要設定失敗都不送出。
    await sleep(400);
    if (!setAspect()) throw new Error(`Google Flow 找不到圖片／影片比例 ${config.aspect || "（未指定）"}`);
    await sleep(300);
    if (isImageMode) {
      if (config.imageModel && !(await setImageModel())) throw new Error(`Google Flow 找不到圖片模型 ${config.imageModel}`);
      await sleep(300);
      ...
      if (wantReuse && prevImage) {
        ...
        if (ok) {
          if (!setImageMode("last")) throw new Error("Google Flow 找不到上一張圖片設定");
          await sleep(500);
        } else {
      ...
      if (!(await setModel())) throw new Error(`Google Flow 找不到影片模型 ${config.model || "（未指定）"}`);
      await sleep(300);
    }
    const outputCount = parseInt(config.outputCount) || 1;
    if (!setOutputs(outputCount)) throw new Error(`Google Flow 找不到生成張數 x${outputCount}`);
    await sleep(300);
    if (!isImageMode) {
    ...
    await sleep(500);
    await tryAddMatchedAssets(item.text, item.assetNames || []);

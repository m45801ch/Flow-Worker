...
export function autoFlowWaitMs(batch: AutoFlowBatch, random = Math.random()): number {
  const min = Math.max(0, Math.min(batch.config.waitMin || 0, batch.config.waitMax || 0));
  const max = Math.max(min, batch.config.waitMax || 0);
  const ratio = Math.min(1, Math.max(0, Number.isFinite(random) ? random : 0));
  return Math.round((min + ratio * (max - min)) * 1000);
}
...
export function handleAutoFlowItemStatus(run: AutoFlowRun, itemId: number, status: "running" | "done" | "error", error?: string): { run: AutoFlowRun; events: AutoFlowRunEvent[] } {
  ...
  if (nextIndex >= run.batches.length) {
    if (rememberedError) {
      const failedRun = { ...run, itemStatuses: nextStatuses, status: "failed" as const, error: rememberedError };

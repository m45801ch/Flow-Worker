...
import { createAutoFlowRun, dispatchCurrentBatch, handleAutoFlowItemResult, handleAutoFlowItemStatus, handleAutoFlowRetry } from "./auto-flow-runner";
...
describe("Auto-Flow run state", () => {
  ...
  it("pauses later Segment Cuts when an earlier Cut fails", () => {
    ...
    expect(result.events.some((event) => event.kind === "dispatch")).toBe(false);
  });
